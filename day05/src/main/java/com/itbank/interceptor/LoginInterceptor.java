...
// 스프링4에서는 extends HandlerInterceptorAdaptpr
// 스프렝5에서는 implements HandlerInterceptor
// HandlerInterceptor는 인터페이스 이지만, 내부 함수가 default 키워드로 처리되어 있다
...
public class LoginInterceptor implements HandlerInterceptor {
	...
	// 1. preHandle : 요청이 컨트롤러의 함수(Handle)에 도달하기 전에 작동
	// 2. postHandle : 핸들이 실행된 이후 view(jsp)로 넘어가기 전에 작동
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		...
		// JSP로 forward가 이루어지기 전에, request, cookie, session 등에서 필요한 값을 확인하고 판별한다
		// ModelAndView를 이용하여 특정 데이터를 넣어주는데 활용할 수 있다
		// header.jsp 를 사용할 수도 있으나, 모든 페이지에 영향을 주기 때문에 일부 페이지만 적용하기 쉽지않다
	...
	public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex)
		...
		// @ExcetionHandler, @ControllerAdvice 가 예외를 처리하는데 특화되어 있다
